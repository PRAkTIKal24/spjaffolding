...
app = typer.Typer(
    help="The `spjaffolding` wizard for advanced ML tool scaffolding."
)
console = Console()
...
AVAILABLE_FEATURES = [
    questionary.Choice(
        "🖥️  GPU / Hardware Acceleration (torch, cuml)", value="gpu"
    ),
    questionary.Choice(
        "📊 Data Visualization (matplotlib, seaborn)", value="viz"
    ),
    questionary.Choice(
        "🧹 Data Preparation Pipeline (creates data_prep/)", value="data_prep"
    ),
    questionary.Choice(
        "🧠 Model Training (creates models/ & training/)",
        value="model_training",
    ),
    questionary.Choice("🕸️  Web Scraping (creates scraping/)", value="scraping"),
    questionary.Choice(
        "🗄️  Vector Database / RAG (creates vector_db/)", value="vector_db"
    ),
    questionary.Choice(
        "🤖 LLM Orchestration (creates llm_orchestration/)",
        value="llm_orchestration",
    ),
]
...
def load_presets() -> dict[str, list[str]]:
    ...
    if presets_file.exists():
        try:
            ...
            console.print(
                f"[bold red]Failed to load custom presets: {e}[/bold red]"
            )
    return presets
...
def save_preset(name: str, features: list[str]) -> None:
    ...
    try:
        ...
        console.print(
            f"[bold green]Saved preset '{name}' to {presets_file}.[/bold green]"
        )
    except Exception as e:
...
def main(
    ...
    if preset:
        ...
        console.print(
            f"[bold blue]Using preset '{preset}': {selected_features}[/bold blue]"
        )
    else:
    ...
    console.print(
        f"[bold green]Starting scaffolding for: {tool_name}[/bold green]"
    )
    try:
        ...
        console.print(
            f"[bold blue]Successfully scaffolded {tool_name}![/bold blue]"
        )
    except Exception as e:
        console.print(
            f"[bold red]Generation encountered an error: {e}[/bold red]"
        )
        raise typer.Exit(code=1)
